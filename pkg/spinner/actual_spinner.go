...
func NewActualSpinner(cs []string, d time.Duration, options ...Option) *ActualSpinner {
	s := &ActualSpinner{
		Delay:       d,
		chars:       cs,
		color:       color.New(color.FgBlack & color.Bold).SprintFunc(),
		mu:          &sync.RWMutex{},
		Writer:      color.Output,
		active:      false,
		stopChan:    make(chan struct{}, 1),
		confirmChan: make(chan struct{}, 1),
		repaintChan: make(chan struct{}, 1),
	}
...
func (s *ActualSpinner) erase() {
	...
	// }
	fmt.Fprintf(s.Writer, "\r\033[K\r") // erases to end of line
	s.lastOutput = ""
}
