...
Sometimes you want to handle the failures yourself, rather than having a script abort the whole process.

Error handling in Rundown is very simple, and only supports two options:

1. Abort the document with a custom message
2. Continue the document with an optional message

There are also two ways to handle this:

1. By exit code
2. By regexp on STDERR

### Handling non-zero exit codes

Any non-zero exit code is considered a failure. There's two options to handle these:

* Using the `skip-on-failure` attribute of the execution block will skip to the next heading at the current level or higher.
* Using a `on-failure` attribute of a separate rundown block to match STDERR on regexp.

#### Using skip-on-failure <r section="fail:skip" />

~~~ markdown
## Some heading

<r spinner="Running..." skip-on-failure />
...
## Some other heading <r section="next" />
